...
const popuCloseByClickOnOverlay = (event) => {
  console.log({
    target: event.target,
...
popup.addEventListener("click", popuCloseByClickOnOverlay);


// ---------------------------------------------------------------------------------------------------------------------------- popup формы добавления фотокарточек

let popupToggleCard = function () {
  popupPlusCard.classList.toggle("popup_opened");
}; // функция для добавления/удаления класса в popup Plus
popupAddCard.addEventListener("click", popupToggleCard); // вызов функции при клике по плюсу для добавления класса
popupCloseCard.addEventListener("click", popupToggleCard); // вызов функции при клике на крестик для удаления класса

// ---------------------------------------------------------------------------------------------- добавление template, добавление и удаление карточки, клик по сердцу
...
// --------------------------------------------------------------------------------------------------------------------------- popup формы фотокарточки на весь экран

let popupCard = document.querySelector(".card"); // popup для фотокарточки на полный экран
let OpenCard = document.querySelector(".element__image"); // фотокарточка
let titleCard = document.querySelector(".element__title"); //  заголовок фотокарточки
let popupBigCard = document.querySelector(".popup__image-element"); // фотокарточка в попапе на весь экран
let popupParagraphCard = document.querySelector(".popup__plase"); // параграф в попапе
let popupIcon = document.querySelector(".popup__icon-click_card"); //

let popupToggleCardPhoto = function () {
  popupCard.classList.toggle("popup_opened");
}; // функция для добавления/удаления класса в popup Card
OpenCard.addEventListener("click", popupToggleCardPhoto); // вызов функции при клике по фотокарточке для добавления класса
popupIcon.addEventListener("click", popupToggleCardPhoto); // вызов функции при клике по фотокарточке для удаления класса

function ClickPopapCard() {
  popupParagraphCard.textContent = titleCard.textContent; // текст из заголовка карточки в параграф попапа
  popupParagraphCard.src = titleCard.src; // текст из src картинки в profile в src картинки попапа
  popupToggleCardPhoto();
} // функция сохранения данных с src и alt картинки при каждом открытии попапа
